...
import PopoverInstance from '@components/popoverInstance'
import indexManager from '@utils/indexManager'
import { sliceClass } from '@utils/helper'

import { PopoverHandleOptions, ArrowSize } from './type'
...
export default class PopoverHandle extends Base<HTMLDivElement> {
  ...
  private init() {
    ...
    this._root.addEventListener('click', (e) => e.stopPropagation())
  ...
  setArrowSize(arrowSize: ArrowSize) {
    ...
    const allClassNames: `is-${ArrowSize}`[] = ['is-large', 'is-middle', 'is-none', 'is-small']
