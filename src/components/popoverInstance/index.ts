...
export default class PopoverInstance {
  ...
  private listenerCb: () => void

  constructor(target?: VirtualElement, popper?: HTMLElement, option?: PopoverOption) {
    this.target = target
    this.popper = popper
  ...
  updateConfig(target?: VirtualElement, popper?: HTMLElement, option?: PopoverOption) {
    this.target = target
    this.popper = popper
    this.option = option || { placement: 'bottom' }
    this.forceUpdate()
